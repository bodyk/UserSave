...
using UserSave.Authentication.Helpers;
...
namespace UserSave.Authentication
{
    public class TokenAuthenticateAttribute : Attribute, IAuthenticationFilter
    {
        ...
        public async Task AuthenticateAsync(HttpAuthenticationContext context, CancellationToken cancellationToken)
        {
            try
            {
                IAuthService authService = context.ActionContext.ControllerContext.Configuration
                ...
                bool correctToken = authService != null && await authService.ValidateTokenAsync(authorization.Parameter);
                if (!correctToken)
                {
